...
function update_lw(u, cfl, unew)
    unew[1] = u[1] - 0.5*cfl*(u[2] - u[end-1]) + 0.5*abs2(cfl)*(u[end-1] - 2*u[1] + u[2])   # u[0] = u[end-1] due to periodic bc
...
function exact_solution(x, N, t)
    exact_sol = fill(0.0, N)
    for i in 1:N
        exact_sol[i] = sin(2.0 * π * (x[i]-t))
    end
    return exact_sol
end

exact_sol = exact_solution(x, N, t)
